...
if __name__=="__main__":
    ...
    pointnet_classifier = PointNetDenseCls(k=3, feature_transform=opt.feature_transform) #Kはセグメンテーションの数、右手左手非接触なので3
    sk_parts_classifier = ContrastiveNet()
    parts2pts_classifier = PartsToPtsNet()


    if opt.model != '':
        pointnet_classifier.load_state_dict(torch.load(opt.model))
    ...
    for epoch in range(opt.nepoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            #model save 
            if best_partseg_acc < partseg_acc:
                print("-----------")
                print("best_acc_partsegを更新")
                ...
                best_partseg_acc=partseg_acc

            if loss_partseg < min_loss_partseg:
                print("----------")
                print("min_loss_partsegを更新")
            ...
            #parts 2 ges
            if ps_loss < min_loss_contrastive:
                print("----------")
                print("min_loss_contrastiveを更新")
                ...
                min_loss_contrastive=ps_loss
            
            #ges2pts
            if loss_p2pts < min_loss_p2pts:
                print("----------")
                print("min_loss_contrastiveを更新")
    ...
    for i,data in tqdm(enumerate(testdataloader, 0)):
        ...
        pointnet_classifier = pointnet_classifier.eval()
        pred, _, _ = pointnet_classifier(points)
